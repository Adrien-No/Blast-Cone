...
amd = {'region': {"br": "br1", "eun": "eun1", "euw": "euw1", "jp": "jp1", "kr": "kr",
...
async def on_raw_reaction_add(payload):
    ...
    if payload.member.bot == False:
        ...
        embed2 = message.embeds[0].add_field(
            name='zebi', value='pump it up', inline=True)
...
async def embed_sum(sum_name):
    ...
    # ...
    embed = discord.Embed(title="link to op.gg", url=f"https://euw.op.gg/summoner/userName=sum_name",
                          description="rank in soloQ and xxx mastery points with actually champ")
    embed.set_author(
        name=sum_name, icon_url="http://ddragon.leagueoflegends.com/cdn/11.3.1/img/profileicon/588.png")
    embed.set_thumbnail(
        url=f"http://ddragon.leagueoflegends.com/cdn/img/champion/splash/{played_champ}_0.jpg")
...
async def game_info(ctx, summoner_name.lower(), region='euw'):
    ...
    embed_menu = discord.Embed(
        title=f"List of summoners in {amd['profile_{summoner_name}']['name']}'s game :")
    # on crée un dic comprenant les participants
    participants = {}
    for i_sum in range(10):
        participants[amd['spectator_{summoner_name}']
                     ['participants'][i_sum]['summonerName']]
    ...
    listEmojisNumbers = [emojis[':one:'], emojis[':two:'], emojis[':three:'], emojis[':four:'],
                         emojis[':five:'], emojis[':six:'], emojis[':seven:'], emojis[':eight:'], emojis[':nine:'], '🔟']
